...
import os
import time
from testsuits.test_calc_table import TestCalcTable
from unit.HTMLTestRunner import HTMLTestRunner


# unittest suite 批量添加测试用例，逐个进行执行
suite = unittest.TestSuite()
suite.addTest(TestCalcTable('test_calc_table'))
...
if __name__ == '__main__':
    ...
    with open(HtmlFile,"wb") as report:
        runner = HTMLTestRunner(stream=report,title=my_title,description=u"测试结果")
        runner.run(suite)
    # print(suite)
    # for i in suite:
    #     print(i)
